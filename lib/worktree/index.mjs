...
async function statusPorcelain(cwd, options = {}) {
  return (await git(cwd, ["status", "--porcelain"], { ...options, cwd })).stdout;
}
...
export async function archiveRequirementWorktree(input = {}) {
  ...
  return await withRequirementLock(projectRoot, requirementId, input, async () => {
    ...
    const canonicalStatus = await statusPorcelain(projectRoot, input);
    if (canonicalStatus.trim()) {
      const result = escalation("canonical_dirty", {
        requirementId,
        path: projectRoot,
        porcelain: canonicalStatus
      });
      await appendEscalationEvent(projectRoot, requirementId, result.reason, result, input);
      return result;
    }
    ...
    const warnings = [];
    if (runtimeState.base_sha) {
      const baseExists = await git(
        projectRoot,
        ["cat-file", "-e", `${runtimeState.base_sha}^{commit}`],
        { ...input, allowFailure: true }
      );
      if (baseExists.exitCode !== 0) {
        ...
        const ancestor = await git(
          projectRoot,
          ["merge-base", "--is-ancestor", runtimeState.base_sha, targetBranch],
          { ...input, allowFailure: true }
        );
    ...
    const mergeResult = await git(projectRoot, ["merge", "--no-edit", workspace.branch], {
      ...input,
      allowFailure: true
    });
    ...
    await writeRuntimeState(projectRoot, requirementId, archivedState, "archive");
    await appendWorktreeEvent(
      projectRoot,
      requirementId,
