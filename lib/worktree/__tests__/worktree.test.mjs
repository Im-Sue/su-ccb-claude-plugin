...
test("merge cleanup reopen and discard reject multi-space runtime without rewriting it", async () => {
  const fixture = await createMultiRepoFixture();
  const requirementId = "req-multispace-guard";
  try {
    await ensureRequirementWorktree({
      projectRoot: fixture.projectRoot,
      requirementId,
    ...
    const statePath = requirementWorktreeStatePath(fixture.projectRoot, requirementId);
    ...
    for (const helper of [
      ...
      await assert.rejects(
        ...
        /multi-space runtime requires the multi-space orchestrator/
      );
