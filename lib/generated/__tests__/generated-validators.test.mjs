...
import { validateRequirementMdFrontmatter } from "../../requirement-md-frontmatter/generated-validator.mjs";
...
test("plugin generated docs-structure-contract validator rejects missing deep required registry fields", () => {
  const contract = validDocsStructureContract();
  const missingEntityStatus = validateDocsStructureContract({
    ...contract,
    entity_status: {
      ...contract.entity_status,
      requirement: {}
    }
  });
  assert.equal(missingEntityStatus.ok, false);
  assert.equal(missingEntityStatus.issues.some((issue) => issue.path === "entity_status.requirement.doc_types"), true);
  assert.equal(missingEntityStatus.issues.some((issue) => issue.path === "entity_status.requirement.values"), true);

  const missingDocumentGroup = validateDocsStructureContract({
    ...contract,
    documents: {
      ...contract.documents,
      requirement_bound: {}
  ...
  assert.equal(missingDocumentGroup.ok, false);
  assert.equal(missingDocumentGroup.issues.some((issue) => issue.path === "documents.requirement_bound.doc_types"), true);
  assert.equal(missingDocumentGroup.issues.some((issue) => issue.path === "documents.requirement_bound.status"), true);
});

test("plugin generated requirement frontmatter validator accepts status-only and requires complete analysis metadata", () => {
  const statusOnly = validateRequirementMdFrontmatter({ status: "cancelled" });
  assert.equal(statusOnly.ok, true);

  const analyzed = validateRequirementMdFrontmatter({
    status: "planning",
  ...
  const partial = validateRequirementMdFrontmatter({
    status: "planning",
    analysis_input_hash: "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
  });
  assert.equal(partial.ok, false);
  assert.equal(partial.issues.some((issue) => issue.path === "analysis_applied_at"), true);
});
