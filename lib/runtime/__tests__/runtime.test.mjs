...
import {
  ...
  ValidationError,
  acquireFileLock,
  appendCollaborationProfileDecided,
  appendEvent,
  appendRiskEnvelopeUpdated,
  safeWriteFile,
...
test("collaboration EventJournal helpers write typed events and preserve idempotency", async () => {
  const projectRoot = await tempProject();
  try {
    const profileEvent = {
      subject_type: "subtask",
      subject_id: "subtask-1",
      idempotency_key: "collaboration-profile:subtask-1:planned",
      emitted_at: "2026-06-22T10:00:00.000Z",
      source_actor: "ccb_claude",
      payload: {
        tier: "lite",
        pass: "planned",
        risk_basis: "Only additive docs changes; no hard-list surfaces touched.",
        negative_evidence: ["no *-api diff", "no SQL migration"],
        waivers: [],
        verification_minimum: "targeted",
        classifier_coverage: "full",
        semantic_overrides: [],
        evidence_refs: ["diff:abc123"]
      }
    };
    ...
    const envelopeEvent = {
      subject_type: "subtask",
      subject_id: "subtask-1",
      idempotency_key: "risk-envelope:subtask-1:api-v1:closed",
      emitted_at: "2026-06-22T10:01:00.000Z",
      source_actor: "ccb_claude",
      payload: {
        surface_ref: { type: "api", id: "GET /api/topics" },
        status: "closed",
        close_evidence: {
          contract_test_ref: "test:api-contract",
          backward_compat_ref: "review:compat-ok"
        }
      }
    };

    const firstProfile = await appendCollaborationProfileDecided(profileEvent, { projectRoot });
    const secondProfile = await appendCollaborationProfileDecided(profileEvent, { projectRoot });
    const envelope = await appendRiskEnvelopeUpdated(envelopeEvent, { projectRoot });

    assert.equal(firstProfile.appended, true);
    assert.equal(secondProfile.appended, false);
    assert.equal(secondProfile.duplicate, true);
    assert.equal(envelope.appended, true);

    const journal = await readFile(join(projectRoot, "docs", ".ccb", "events", "journal.jsonl"), "utf8");
    const events = journal.trim().split(/\r?\n/).map((line) => JSON.parse(line));
    assert.deepEqual(events.map((event) => event.type), ["collaboration_profile_decided", "risk_envelope_updated"]);
    assert.equal(events[0].payload.risk_basis, profileEvent.payload.risk_basis);
    assert.deepEqual(events[1].payload.surface_ref, envelopeEvent.payload.surface_ref);
    ...
    await assert.rejects(
      () =>
        appendCollaborationProfileDecided(
          {
            ...profileEvent,
            idempotency_key: "collaboration-profile:bad",
            payload: { ...profileEvent.payload, tier: "unsafe-lite" }
          },
          { projectRoot }
        ),
      ValidationError
