...
test("SQL/MyBatis L2 redline fixtures are classified and do not remain partial", async () => {
  const corpus = await loadClassifierCorpus();
  for (const id of ["l2_alter_add_not_null_default_standard", "l2_create_unique_index_standard", "l2_sql_sink_table_write_full"]) {
...
test("Spring route/signature L2-B redline fixtures are classified", async () => {
  const corpus = await loadClassifierCorpus();
  for (const id of ["l2_shadow_route_standard", "l1_existing_method_signature_breaking"]) {
    const item = corpus.cases.find((candidate) => candidate.id === id);
    assert.ok(item, `missing corpus case ${id}`);
    const result = classifyGitDiff(item.diff);
    assert.notEqual(result.minimum_tier, "lite", `${id} must not be lite`);
    assert.equal(result.classifier_coverage, "full", `${id} should be resolved by route/signature analyzer`);
    assert.equal(result.coverage.partial_reasons.length, 0, `${id} should not leave partial coverage`);
  }

  const additive = corpus.cases.find((candidate) => candidate.id === "l2_new_endpoint_additive_lite");
  assert.ok(additive, "missing corpus case l2_new_endpoint_additive_lite");
  const result = classifyGitDiff(additive.diff);
  assert.equal(result.minimum_tier, "lite", "pure new endpoint should remain lite");
  assert.equal(hasSurface(result, { type: "api", change: "additive", floor: "lite", source: "l2.route.additive_endpoint" }), true);

  const arrayRouteDiff = [
    ...
    "@@ -1,7 +1,9 @@",
    " @RequestMapping(value = \"/market/topic\")",
    " public class TopicController {",
    "   @GetMapping(\"/export\")",
    "   public TopicRespVO exportTopic() { return null; }",
    "+  @GetMapping(value = {\"/{id}\", \"/detail/{id}\"})",
    "+  public TopicRespVO getById(Long id) { return null; }",
    " }"
  ].join("\n");
  const arrayRoute = classifyGitDiff(arrayRouteDiff);
  assert.notEqual(arrayRoute.minimum_tier, "lite", "array route with path variable shadow should not be lite");
  assert.equal(hasSurface(arrayRoute, { type: "api", change: "shadow_route", floor: "standard", source: "l2.route.shadow_route" }), true);
});

test("sink graph/config/auth/enum L2-C redline fixtures are classified", async () => {
  const corpus = await loadClassifierCorpus();
  for (const id of [
    "l2_sink_service_implementation_full",
    "l2_sink_api_consumer_standard",
    "l2_mq_indirect_money_standard",
    "l2_sensitive_money_config_standard",
    "l2_auth_annotation_full",
    "l2_enum_state_machine_standard"
  ]) {
    const item = corpus.cases.find((candidate) => candidate.id === id);
    assert.ok(item, `missing corpus case ${id}`);
    const result = classifyGitDiff(item.diff);
    assert.notEqual(result.minimum_tier, "lite", `${id} must not be lite`);
    assert.equal(result.classifier_coverage, "full", `${id} should be classified by L2-C`);
    assert.equal(result.coverage.partial_reasons.length, 0, `${id} should not leave partial coverage`);
  }

  const unresolved = corpus.cases.find((candidate) => candidate.id === "l2_reflection_unresolved_sink_standard");
  assert.ok(unresolved, "missing corpus case l2_reflection_unresolved_sink_standard");
  const unresolvedResult = classifyGitDiff(unresolved.diff);
  assert.equal(unresolvedResult.classifier_coverage, "partial");
  assert.equal(unresolvedResult.minimum_tier, "standard");
  assert.equal(
    unresolvedResult.coverage.partial_reasons.some((reason) => reason.surface === "unresolved_sink_call_chain"),
    true
  );

  const readOnly = corpus.cases.find((candidate) => candidate.id === "l2_sink_read_call_lite");
  assert.ok(readOnly, "missing corpus case l2_sink_read_call_lite");
  const readOnlyResult = classifyGitDiff(readOnly.diff);
  assert.equal(readOnlyResult.classifier_coverage, "full");
  assert.equal(readOnlyResult.minimum_tier, "lite");
  assert.equal(hasSurface(readOnlyResult, { type: "money_sink", change: "sink_read_call", floor: "lite", source: "l2.sink.read_call" }), true);

  const deleteSinkTable = classifyGitDiff(
    [
      "diff --git a/sql/cleanup.sql b/sql/cleanup.sql",
      "index 1111111..2222222 100644",
      "--- a/sql/cleanup.sql",
      "+++ b/sql/cleanup.sql",
      "@@ -1 +1 @@",
      "+DELETE FROM point_account WHERE user_id = 1001;"
    ].join("\n")
  );
  assert.equal(deleteSinkTable.minimum_tier, "full");
  assert.equal(hasSurface(deleteSinkTable, { type: "money_sink", change: "sink_table_write", floor: "full", source: "l2.sink.table_write" }), true);
});
