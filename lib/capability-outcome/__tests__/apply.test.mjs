...
async function seedTerminalTaskStates(projectRoot, taskKeys, overrides = {}) {
  for (const taskKey of taskKeys) {
    ...
    await writeFile(path, [
      ...
      "priority: medium",
      "requirement_id: req-finalize",
      "section_id: pr1-finalize",
      "order: 1",
      "implementation_owner: ccb_codex",
      "dependencies: []",
      "source_breakdown_draft: docs/.ccb/drafts/breakdown/req-finalize.json",
...
test("requirement.finalize writes delivered through requirement markdown with terminal scope evidence", async () => {
  ...
  try {
    const requirement = await writeRequirementMarkdown(projectRoot, requirementId, "planning");
    await seedTerminalTaskStates(projectRoot, taskKeys);
