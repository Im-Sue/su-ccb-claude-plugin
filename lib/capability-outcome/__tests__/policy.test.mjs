...
test("generated policy resolves cancel guards and retired analyze policy is absent", () => {
  ...
  const requirementReactivate = resolveCapabilityOutcomePolicy({
    capabilityId: "requirement.reactivate",
    outcomeType: "planning",
    subjectType: "requirement"
  });
  assert.equal(requirementReactivate.write_target, "requirement_md");
  assert.equal(requirementReactivate.state_effects.status, "set:planning");
  assert.deepEqual(requirementReactivate.must_ask_refs, ["must_ask_9"]);
  assert.ok(requirementReactivate.guards.includes("requirement_reactivate_status_protection"));
