...
test("generated policy resolves requirement.finalize with terminal scope evidence", () => {
  const policy = resolveCapabilityOutcomePolicy({
  ...
  assert.deepEqual(policy.evidence_required.items.map((item) => item.check_id), ["dev_task_scope_terminal"]);
...
test("policy validator accepts dev_task_scope_terminal as fixed check id", () => {
    ...
    policies: [
      {
        ...
        evidence_required: {
          mode: "all",
          items: [{ kind: "C", source: "dev_task_scope", check_id: "dev_task_scope_terminal" }]
        },
        must_ask_refs: []
      }
    ]
