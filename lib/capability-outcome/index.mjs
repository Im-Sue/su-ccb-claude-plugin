...
const REQUIREMENT_REACTIVATE_POLICY_ID = "requirement.reactivate:planning:requirement";
...
async function validateRequirementReactivateGuards(input) {
  const checked = await readRequirementForGuard({ ...input, capabilityName: "requirement.reactivate" });
  if (!checked.ok) return checked;

  if (checked.status === "deferred" || checked.status === "cancelled") return { ok: true };
  if (checked.status === "planning") return { ok: true, noop: true };

  return {
    ok: false,
    code: "GUARD_FAILED",
    issues: [`requirement.reactivate can only reactivate cancelled/deferred requirement, current status is ${checked.status ?? "missing"}`]
  };
}
...
async function validateExecutableGuards({ projectRoot, policy, subjectRef, expectedHash }) {
  ...
  if (policy.policy_id === REQUIREMENT_REACTIVATE_POLICY_ID) {
    return await validateRequirementReactivateGuards({ projectRoot, subjectRef, expectedHash });
  }
