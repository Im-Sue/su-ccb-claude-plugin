...
test("deferRequirement writes through governed capability outcome and EventJournal audit", async () => {
  const projectRoot = await tempProject();
  const requirementId = "req-defer-lib";
  try {
    ...
    const result = await deferRequirement({
      ...
      dispatchRef: { jobId: "job-defer-lib" },
      now: "2026-06-19T10:00:00.000Z"
    });

    assert.equal(result.ok, true);
    assert.equal(result.noop, false);
    assert.equal(result.capability_id, "requirement.defer");
    assert.equal(await readRequirementStatus(projectRoot, requirementId), "deferred");

    const events = await readEvents(projectRoot);
    assert.ok(events.some((event) =>
      event.type === "user_lifecycle_authorized" &&
      event.subject_id === requirementId &&
      event.payload.action === "defer"
    ));
    assert.ok(events.some((event) =>
      event.type === "capability_outcome_applied" &&
      event.payload.capability_id === "requirement.defer" &&
      event.payload.outcome_type === "deferred"
    ));
    assert.ok(events.some((event) =>
      event.type === "state_write_intent" &&
      event.payload.operation === "applyCapabilityOutcome" &&
      event.payload.resource_type === "requirement_md"
    ));
...
test("deferRequirement returns CAS_CONFLICT and preserves changed requirement", async () => {
  const projectRoot = await tempProject();
  const requirementId = "req-defer-cas";
  try {
    await writeRequirement(projectRoot, requirementId, "planning");
    const result = await deferRequirement({
      projectRoot,
      requirementId,
      dispatchRef: { jobId: "job-defer-cas" },
      testHooks: {
        beforeRequirementDeferApply: async ({ requirement }) => {
          await writeFile(
            requirement.path,
            requirement.content.replace("status: planning", "status: delivering"),
            "utf8"
          );
        }
      }
    ...
    assert.equal(await readRequirementStatus(projectRoot, requirementId), "delivering");
    assert.ok((await readEvents(projectRoot)).some((event) =>
      event.type === "capability_outcome_rejected" &&
      event.payload.capability_id === "requirement.defer" &&
      event.payload.code === "CAS_CONFLICT"
    ));
  } finally {
    await rm(projectRoot, { recursive: true, force: true });
  }
...
test("reactivateRequirement writes cancelled/deferred requirement to planning through governed path", async () => {
  for (const status of ["cancelled", "deferred"]) {
    const projectRoot = await tempProject();
    const requirementId = `req-reactivate-${status}`;
    try {
      await writeRequirement(projectRoot, requirementId, status);
      const result = await reactivateRequirement({
        projectRoot,
        requirementId,
        reason: "resume delivery",
        sourceActor: "ccb_claude",
        dispatchRef: { jobId: `job-reactivate-${status}` },
        now: "2026-06-19T10:05:00.000Z"
      });

      assert.equal(result.ok, true);
      assert.equal(result.noop, false);
      assert.equal(result.capability_id, "requirement.reactivate");
      assert.equal(await readRequirementStatus(projectRoot, requirementId), "planning");
      const events = await readEvents(projectRoot);
      assert.ok(events.some((event) =>
        event.type === "user_lifecycle_authorized" &&
        event.subject_id === requirementId &&
        event.payload.action === "reactivate"
      ));
      assert.ok(events.some((event) =>
        event.type === "capability_outcome_applied" &&
        event.payload.capability_id === "requirement.reactivate" &&
        event.payload.outcome_type === "planning"
      ));
    } finally {
      await rm(projectRoot, { recursive: true, force: true });
    }
...
test("reactivateRequirement leaves planning requirement as governed no-op", async () => {
  const projectRoot = await tempProject();
  const requirementId = "req-reactivate-planning-noop";
  try {
    await writeRequirement(projectRoot, requirementId, "planning");
    const result = await reactivateRequirement({
      projectRoot,
      ...
      sourceActor: "ccb_claude",
      dispatchRef: { jobId: "job-reactivate-planning-noop" },
      now: "2026-06-19T10:07:00.000Z"
    });
...
test("reactivateRequirement guard rejects active or delivered requirements", async () => {
  for (const status of ["drafting", "delivering", "delivered"]) {
    const projectRoot = await tempProject();
    const requirementId = `req-reactivate-guard-${status}`;
    try {
      await writeRequirement(projectRoot, requirementId, status);
      const result = await reactivateRequirement({
        projectRoot,
        requirementId,
        dispatchRef: { jobId: `job-reactivate-guard-${status}` }
      });

      assert.equal(result.ok, false);
      assert.equal(result.code, "GUARD_FAILED");
      assert.match(result.issues.join("\n"), /can only reactivate cancelled\/deferred requirement/);
      assert.equal(await readRequirementStatus(projectRoot, requirementId), status);
    } finally {
      await rm(projectRoot, { recursive: true, force: true });
    }
  }
